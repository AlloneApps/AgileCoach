                    ...
                    if (snapshot.exists()) {
                        ...
                        if (mobileNumber != null && mobilePin != null) {
                            if (userMobileNumber.equals(mobileNumber) && mobilePin.equals(mPin)) {
                                ...
                                if (snapshot.child(FireBaseDatabaseConstants.IS_ACTIVE).getValue(String.class).equals("false")) {
                                    isActive = false;
                                }
                                if (isActive) {
                                    User user = new User();
                                    ...
                                    hideProgressDialog();
                                    MyTasksToast.showErrorToastWithBottom(context, mobileNumber + " is not activated or deActivated, Please contact admin.", MyTasksToast.MYTASKS_TOAST_LENGTH_LONG);
                                }
                            } else {
                                hideProgressDialog();
                                MyTasksToast.showErrorToastWithBottom(context, "Credential mismatch.", MyTasksToast.MYTASKS_TOAST_LENGTH_SHORT);
                            }
                        } else {
                            hideProgressDialog();
                        ...
                        hideProgressDialog();
                        MyTasksToast.showErrorToastWithBottom(context, "Failed to login, verify credentials", MyTasksToast.MYTASKS_TOAST_LENGTH_SHORT);
                    }
                }
